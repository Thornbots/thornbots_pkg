...
Don't front-run firing logic unless asked.
...
- **Real firing logic is not built** — no HP/heat/power gating, no timing.
  ...
  never consults the aim solve, so it can fire when the TF lookup has failed.
  An aim/lead controller does exist (`target_tracker.py` plus
